...
void *ns_start_thread(void *(*f)(void *), void *p) {
	...
	pt_result = pthread_attr_init(&attr);
	if (pt_result != 0)
	{
		syslog(LOG_NOTICE, "[ns_start_thread] Error pthread_attr_init result = %d", pt_result);
	}
	else
	{
		pt_result = pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
		if (pt_result != 0)
		{
			syslog(LOG_NOTICE, "[ns_start_thread] Error pthread_attr_setdetachstate result = %d", pt_result);
		}
		else
		{
#if defined(NS_STACK_SIZE) && NS_STACK_SIZE > 1
			pt_result = pthread_attr_setstacksize(&attr, NS_STACK_SIZE);
			if (pt_result != 0)
			{
				syslog(LOG_NOTICE, "[ns_start_thread] Error pthread_attr_setstacksize result = %d", pt_result);
			}
#endif
			pt_result = pthread_create(&thread_id, &attr, f, p);
			if (pt_result != 0)
			{
				syslog(LOG_NOTICE, "[ns_start_thread] Error pthread_create result = %d", pt_result);
				thread_id = 0;
			}
...
#define MONGOOSE_IDLE_TIMEOUT_SECONDS 30 // FlylinkDC++
#endif
